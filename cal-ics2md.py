...
def generate_event(event):
    ...
    return f'<details><summary>{summary}</summary>Waktu: {start_time} - {end_time}</details>'
...
for event in cal.walk('vevent'):
    data.append([event['summary'], event['dtstart'].dt, event['dtend'].dt])

df = pd.DataFrame(data, columns=['summary', 'dtstart', 'dtend'])
